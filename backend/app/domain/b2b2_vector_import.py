"""Bounded B2-B2 SR profile import; independent of historical and RU importers."""
...
def prepare_reviewed_source(
    ...
    if source["source_id"] not in {"173594", "173904", "170457"}:
    ...
    for raw in source["food_nutrients"]:
        ...
        observation = {
            "audit_identity": f"PR6-DATA-B2-B2-REDESIGNED:{source['source_id']}:{raw['id']}",
    ...
    for field, component in LEGACY_COMPONENTS.items():
        ...
        if amount:
            ...
            if not any(
                v["nutrient_code"] == code and v["amount"] == amount for v in values
            ):
                raise ValueError(
                    "Проекция Nutrition v1 не подтверждена картой вектора."
                )
    return (
        legacy,
