...
Для каждой карты bundle обязан предоставить `raw_card_sha256` без замены при
чтении. `SourceCardInput` сверяет его с UTF-8 `raw_card_text`; отсутствие hash или
несовпадение завершается явной ошибкой. Только путь создания нового bundle
вычисляет hash из полученного текста.

Identity документа: `source_code + source_version + raw_bytes_sha256`.
Точный повтор всех сохраняемых фактов возвращает существующий document ID и
`0` вставок. При той же identity любое расхождение metadata (включая
`retrieved_at`), `raw_text_sha256`, состава/identity карт, raw payload/hash или
структурированных вариантов, ингредиентов и declared nutrients вызывает
`CorpusImportConflictError` до любых записей. Существующая ревизия не обновляется.
Сравнение исключает только storage-owned UUID и `created_at`; коллекции
сопоставляются по сохраняемым ключам, Decimal — по точному числовому значению,
retrieval time — по моменту времени, независимо от записи часового пояса.

Новая source version или изменившиеся raw bytes создают новую document revision.
Повторный захват/обработка тех же bytes с иным payload не создаёт новую ревизию
неявно: сосуществование parser/capture revisions требует отдельного будущего
versioned-контракта и в PR36 не реализуется.
...
При финальной коррекции PR36 проверка обнаружила шесть устаревших card hashes в
bootstrap. Исправлены только эти шесть значений на SHA-256 существующих raw texts;
тексты, structured facts и document metadata сохранены. Это явная коррекция
fixture, а не автоматическое исправление входящих bundles при импорте.
